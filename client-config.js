(() => {
  const C = {
    ...
    // Reviews (from real Google reviews)
    reviews: [
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    setText('.nav__logo-main', 'PET SPA');
    const navCta = $('.nav__cta');
    if (navCta) { navCta.href = waHref; navCta.textContent = 'Book Online'; }
    const navLinkTexts = ['Services', 'About', 'Gallery', 'Contact'];
    $$('.nav__links li a').forEach((a, i) => { if (navLinkTexts[i]) a.textContent = navLinkTexts[i]; });
    ...
    if (srvGrid) {
      srvGrid.innerHTML = C.services.map(s => `
        <div class="service-card${s.accent ? ' service-card--accent' : ''}">
          ...
          <h3>${s.title}</h3>
          <p>${s.desc}</p>
          <a href="${waHref}" target="_blank" rel="noopener" class="service-card__link">
            Book Now →
          </a>
        </div>
      `).join('');
    ...
    setText('.footer__logo-main', 'PET SPA');
    setText('.footer__copy', C.footerCopy);

    const footerLinks = $$('.footer__links a');
    const footerLabelMap = ['Services', 'Gallery', 'Contact'];
    footerLinks.forEach((a, i) => {
      if (a.href.includes('instagram') || a.textContent.includes('Instagram')) {
        a.href = C.instagram;
        a.textContent = 'Instagram →';
      } else if (footerLabelMap[i]) {
        a.textContent = footerLabelMap[i];
      }
    });
